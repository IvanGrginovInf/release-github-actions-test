				...
				<>
						...
						>
							{Object.keys(imageUrl).map(function(keyName) {
								...
								return (
									<BaseControl
										key={keyName}
										label={sprintf(__('Image %s screen size', 'test-theme'), pointLabel)}
									>
										{!_.isEmpty(attributes[attr]) ?
											<>
												...
												>
													{sprintf(__('Remove %s screen size image', 'test-theme'), pointLabel)}
												</Button>
											</> :
											<MediaPlaceholder
												icon="format-image"
					...
					{showImageFull &&
						<ToggleControl
							...
							help={__('If checked, the image will always stretch the full width of the container and ignore its maximum width.', 'test-theme')}
							checked={imageFull}
